...
const deleteUser = (id) => {
    ...
    vex.dialog.confirm({
        ...
        callback: (response) => {
            ...
            ajax.open("GET", window.location.origin+`/quokka/users/delete/?id=${id}`, true);
    ...
    if($('#user-list').length > 0){
        table = $('#user-list').DataTable({			
            ...
            columnDefs: [
                ...
                { className: 'text-center', data: 'cell_phone',     name: 'cell_phone',     targets: 1 }, 
                { className: 'text-left',   data: 'email_address',  name: 'email_address',  targets: 2 }, 
                { className: 'text-center', data: 'acoes',          name: 'acoes',          targets: 3, 
                    render : function (data, type, row) { 
                        let prefix = document.querySelector("#url-user-prefix").href;
                        ...
                        return `
                            <div style="display: flex; align-items: center; justify-content: space-around;">
                                ...
                                <button class="btn-default" onclick="deleteUser(${row.id})" title="Exluir Usuário">
        ...
        form.addEventListener('submit', event => {
            ...
            let formData = new FormData(form),
                objReq = {
                    ...
                    'password_extension':   formData.get('password').length,
                    'gender':               formData.get('gender')
                }   
            ;
    
            ajax.open("GET", window.location.origin+`/quokka/users/save/?${new URLSearchParams(objReq).toString()}`, true);
            ajax.responseType = "text";
            ajax.send();
            ajax.onreadystatechange = function() {
                try {
                    ...
                    switch(data.status){
                        ...
                        case 200:  
                            ...
                            if(window.location.href.indexOf("edit") == -1){
                                setTimeout(() => window.location.href = window.location.origin+`/quokka/users/edit/${data.id}`, 2000);
                            }
                        break;
